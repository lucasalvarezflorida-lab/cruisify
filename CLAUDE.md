...
- The simulated feed is the `aisTick()` block. When replacing with real AIS (AISStream.io WebSocket), keep the same shape: mutate `vessels`, call `setLatLng`, refresh the detail panel if selected.
